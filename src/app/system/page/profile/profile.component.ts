...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.data = JSON.parse(this.storageService.getCookie(environment.admin));
  }
    // update
    openUpdateModal() {
    ...
    confirmUpdate($event) {
      console.log($event);
      this.adminService.updateProfile($event).subscribe(
        response => {
    ...
    changeInfoCookie(data){
      ...
      this.storageService.setCookie(environment.admin, JSON.stringify(this.data));
      location.reload();
    }
